...
#define dim 10 // grid dimension excluding ghost cells
#define STARVE 5
...
void makeGrid(int *grid)
{
    ...
    for (int i = 1; i <= dim; i++)
    {
        for (int j = 1; j <= dim; j++)
        {
            ...
            {
                //since prey dies off the fastest, we have the most of them.
                tot++;
                guy = 1;
            }
            grid[i * (dim + 2) + j] = guy;
...
int randomlyDecideDirection(int *grid, int id)
{
    ...
    for (it = 0; it < itEnd; it++)
    {
        ...
        for (i = 1; i <= dim; i++)
        {
            for (j = 1; j <= dim; j++)
            {
                ...
                dir = randomlyDecideDirection(grid, id);
